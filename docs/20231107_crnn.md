...
## Sequence Labeling
The sequence vector is converted into a 1-dimension vector input $(x_1, x_2, ..., x_T)$ with corresponding labels $(y_1, y_2, ..., y_T)$ and sends it to the LSTM network ([source code](https://github.com/tuongtranngoc/CRNN/blob/main/src/models/neck.py#L35-L45)).
...
## Transcription Layer
Transcription is to find the label sequence with hightest conditional probability on per-frame predictions (outputs from LSTM model) and remove the repeat labels and blanks.
