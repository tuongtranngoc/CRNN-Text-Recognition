...
To get around these problems, CTC use a new character to the set of allowed outputs $\epsilon$ (blank character). The $\epsilon$ character doesn't correspond to anything and is simply removed from the output. The below figure describes steps that how to map to $Y$ after merging repeats and removing `blank` characters. 
